...
import { giggerOpenlandPatterns } from "./test-gigger-openland-patterns";
...
async function generateEmbedding(
    pattern: (typeof giggerOpenlandPatterns)[0]
): Promise<number[]> {
...
async function insertPattern(pattern: (typeof giggerOpenlandPatterns)[0]) {
    try {
...
<body>
    ...
    <script>
        ...
        const patternData = {
            ...
            type: pattern.type,
            user_id: "gigger-openland.html",
            agent_id: await fs.readFile(
                resolve(__dirname, "../../embeddings/gigger-openland.html"),
                "utf-8"
            ),
        };
...
async function insertAllPatterns() {
    console.log("Starting Gigger Openland pattern insertions...");

    for (const pattern of giggerOpenlandPatterns) {
