...
import { glitter4cardGridPatterns } from "./test-glitter-4card-grid-patterns";
...
async function generateEmbedding(
    pattern: (typeof glitter4cardGridPatterns)[0]
): Promise<number[]> {
...
async function insertPattern(pattern: (typeof glitter4cardGridPatterns)[0]) {
    try {
...
<body>
    ...
    <script>
        ...
        const patternData: PatternData = {
            ...
            type: pattern.type,
            user_id: "glitter-4card-grid.html",
            agent_id: await fs.readFile(
                resolve(
                    __dirname,
                    "../../embeddings/templates/glitter-4card-grid.html"
                ),
                "utf-8"
            ),
        };
...
async function insertAllPatterns() {
    console.log("Starting Glitter 4-Card Grid pattern insertions...");

    for (const pattern of glitter4cardGridPatterns) {
