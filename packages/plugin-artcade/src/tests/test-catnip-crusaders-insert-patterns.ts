...
import { catnipCrusadersPatterns } from "./test-catnip-crusaders-patterns";
...
async function generateEmbedding(
    pattern: (typeof catnipCrusadersPatterns)[0]
): Promise<number[]> {
...
async function insertPattern(pattern: (typeof catnipCrusadersPatterns)[0]) {
    try {
...
<body>
    ...
    <script>
        ...
        const patternData = {
            ...
            type: pattern.type,
            user_id: "catnip-crusaders.html",
            agent_id: await fs.readFile(
                resolve(__dirname, "../../embeddings/catnip-crusaders.html"),
                "utf-8"
            ),
        };
...
async function insertAllPatterns() {
    console.log("Starting Catnip Crusaders pattern insertions...");

    for (const pattern of catnipCrusadersPatterns) {
