...
import { airHockeyPatterns } from "./test-air-hockey-patterns";
...
async function generateEmbedding(
    pattern: (typeof airHockeyPatterns)[0]
): Promise<number[]> {
...
async function insertPattern(pattern: (typeof airHockeyPatterns)[0]) {
    try {
...
<body>
    ...
    <script>
        ...
        const patternData = {
            ...
            type: pattern.type,
            user_id: "air-hockey.html",
            agent_id: await fs.readFile(
                resolve(__dirname, "../../embeddings/air-hockey.html"),
                "utf-8"
            ),
        };
...
async function insertAllPatterns() {
    console.log("Starting Air Hockey pattern insertions...");

    for (const pattern of airHockeyPatterns) {
